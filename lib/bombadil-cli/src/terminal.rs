...
use bombadil_terminal::trace::{
    TerminalOutputWriter, TerminalTraceWriter, TraceReader,
};
...
pub enum Command {
    ...
    Test {
        ...
        /// Where to store output data (trace.bin). Defaults to a
        /// fresh temporary directory.
        #[arg(long)]
        output_path: Option<PathBuf>,
        /// Overwrite any existing trace at --output-path. Without this
        /// flag, Bombadil refuses to write when trace.bin already exists.
        #[arg(long)]
        output_path_overwrite: bool,
        /// Reproduce a previous test run from a trace file (file path
        /// or run directory containing `trace.bin`). Replays the recorded
...
fn load_reproduce_actions(
    ...
    for record in TraceReader::open(path)? {
        if let Some(action) = record?.entry.action {
