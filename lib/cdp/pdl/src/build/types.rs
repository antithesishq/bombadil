...
use crate::pdl::{
    Command, DataType, Domain, Event, Item, Param, Type, TypeDef, Variant,
};
...
impl<'a> DomainDatatype<'a> {
    ...
    pub fn type_description_tokens(&self, domain_name: &str) -> TokenStream {
        ...
        let url = match self {
            DomainDatatype::Type(ty) => {
                format!("{}{}/#type-{}", base_url, domain_name, ty.name())
            }
    ...
    pub fn ident_name(&self) -> String {
        match self {
            ...
            DomainDatatype::Command(cmd) => {
                format!("{}Params", cmd.name().to_upper_camel_case())
            }
            DomainDatatype::Event(event) => {
...
impl FieldDefinition {
    ...
    pub fn generate_meta(
        ...
        if self.serde_skip {
            ...
            if self.is_enum {
                serde_attr
                    .extend(SerdeSupport::generate_enum_de_with(self.optional));
            }
        }
