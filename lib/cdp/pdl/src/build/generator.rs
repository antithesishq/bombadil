...
/// # use anyhow::Result;
/// fn main() -> Result<()> {
///   cdp_pdl::build::compile_pdls(&["src/js.pdl", "src/browser.pdl"])?;
///   Ok(())
/// }
/// ```
...
impl Generator {
    ...
    pub fn allowed_deprecated_type(
        &mut self,
    ...
    ///
    /// This method is like the `cdp_pdl::build::compile_pdls`
    ...
    /// # use anyhow::Result;
    /// fn main() -> Result<()> {
    ///   let mut pdl_build = cdp_pdl::build::Generator::default();
    ///   pdl_build.out_dir("some/path");
    ///   pdl_build.compile_pdls(&["src/frontend.pdl", "src/backend.pdl"])?;
    ///   Ok(())
    /// }
    /// ```
    pub fn compile_pdls<P: AsRef<Path>>(&mut self, pdls: &[P]) -> Result<()> {
        let target: PathBuf =
        ...
        if let Some(allowed) = &self.allowed_domains {
            ...
            for protocol in &mut protocols {
                protocol.domains.retain(|d| full.contains(d.name.as_ref()));
            }
        }
        ...
        for (idx, pdl) in protocols.iter().enumerate() {
            ...
            let version =
                format!("{}.{}", pdl.version.major, pdl.version.minor);
        ...
        let stream = quote! {
            ...
            /// `cdp_types::MethodType` and it must be `DeserializeOwned`
            ...
            pub trait CustomEvent: ::std::any::Any + serde::de::DeserializeOwned + cdp_types::MethodType + Send + Sync {
            ...
            pub(crate) mod sealed {
                ...
                pub trait SealedEvent: ArcAny + cdp_types::MethodType {
    ...
    fn generate_type(
        ...
        let stream = if let Some(vars) = dt.as_enum() {
            ...
            let mut stream =
                self.generate_struct(domain, &dt, dt.ident_name(), params);
            ...
            if !dt.is_type() {
                stream.extend(quote! {
                    impl cdp_types::Method for #name {
                    ...
                    impl cdp_types::MethodType for #name {

                        fn method_id() -> cdp_types::MethodId where Self: Sized {
                            Self::IDENTIFIER.into()
                        }
                    }
                });
            ...
            if let DomainDatatype::Command(cmd) = dt {
                let returns_name =
                    format!("{}Returns", cmd.name().to_upper_camel_case());
                let with_deprecated = self.with_deprecated;
                let with_experimental = self.with_experimental;
                ...
                stream.extend(
                    self.generate_struct(
                        ...
                        cmd.returns
                            ...
                            .filter(|p| {
                                with_experimental || !p.is_experimental()
                            }),
                    ),
                );
                ...
                // impl `Command` trait
                let response =
                    format_ident!("{}Returns", dt.name().to_upper_camel_case());
                stream.extend(quote! {
                    impl cdp_types::Command for #name {
    ...
    fn store_size(&mut self, ty: &str, size: Either<usize, String>) {
        match size {
            ...
            Either::Right(name) => {
                self.ref_sizes.push_back((ty.to_string(), name))
            }
        }
    }
        ...
        T: Iterator<Item = &'a Param<'a>> + 'a,
    {
        ...
        for param in params {
            ...
            let field_name =
                format_ident!("{}", generate_field_name(param.name()));

            let (ty, size) = self.generate_field_type(
                domain,
                dt.name(),
                param.name(),
                &param.r#type,
        ...
        if builder.fields.is_empty() {
            if let DomainDatatype::Type(tydef) = dt {
                ...
                let (wrapped_ty, size) = self.generate_field_type(
                    domain,
                    dt.name(),
                    dt.name(),
                    &tydef.extends,
    ...
    fn generate_field_type(
        ...
        match ty {
            ...
            Type::Binary => (
                FieldType::new(quote! {cdp_types::Binary}),
                Either::Left(size_of::<cdp_types::Binary>()),
            ),
            Type::Enum(_) => {
            ...
            Type::ArrayOf(ty) => {
                ...
                let ty = if let Type::Ref(name) = ty.deref() {
                    ...
                    let (ty, _) = self
                        .generate_field_type(domain, parent, param_name, ty);
                    quote! {#ty}
                };
    ...
    fn projected_type(&self, domain: &Domain, name: &str) -> TokenStream {
        ...
        if path.is_empty() {
            ...
            let current_domain_idx =
                self.domains.get(domain.name.as_ref()).unwrap();
            let ref_domain_idx = self.domains.get(&path).unwrap_or_else(|| {
                panic!("No referenced domain found for {path}")
            });
            ...
            if *current_domain_idx == *ref_domain_idx {
                ...
                let mod_name =
                    format_ident!("{}", self.protocol_mods[*ref_domain_idx]);
                let super_ident = format_ident!("{}", path.to_snake_case());
                quote! {
        ...
        for domain in pdls.iter().flat_map(|p| {
                ...
                .filter(|ev| self.with_experimental || !ev.is_experimental())
            {
                let domain_idx = self
                    ...
                    .unwrap_or_else(|| {
                        panic!(
                            "No matching domain registered for {}",
                            domain.name
                        )
                    });
                let protocol_mod =
                    format_ident!("{}", self.protocol_mods[*domain_idx]);

                let ev_name =
                    format!("Event{}", event.name().to_upper_camel_case());

                let size = *self.type_size.get(&ev_name).unwrap_or_else(|| {
                    panic!("No type found for ref {ev_name}")
                });
