    ...
    for (idx, line) in input.lines().enumerate() {
        ...
        if trim_line.starts_with('#') {
            if let Some(desc) = description.as_mut() {
                desc.push('\n');
                desc.extend(
                    trim_line.chars().skip(1).skip_while(|c| c.is_whitespace()),
                );
            } else {
                description = Some(
                    trim_line
        ...
        if let Some(caps) =
            regex!("^(experimental )?(deprecated )?domain (.*)").captures(line)
        {
            if let Some(domain) = protocol.domains.last_mut()
                && let Some(mut element) = element.take()
            {
        ...
        // cmd or event
        if let Some(caps) =
            regex!("^  (experimental )?(deprecated )?(command|event) (.*)")
                .captures(line)
        {
            ...
            if Some("command") == caps.get(3).map(|m| m.as_str()) {
                let cmd = Command {
                    ...
                    redirect: None,
                    raw_name: Cow::Owned(format!("{}.{}", domain.name, name)),
                    is_circular_dep: is_circular_dep(
                        &domain.name,
                        name.as_ref(),
                    ),
                    name,
        ...
        if let Some(caps) =
            regex!("^    (parameters|returns|properties)").captures(line)
        {
            if let Some(member) = member.take() {
                element
                    .as_mut()
        ...
        if let Some(caps) = regex!("^    redirect ([^\\s]+)").captures(line) {
            ...
            if let Some(desc) = description.as_ref()
                && let Some(caps) =
                    regex!("^Use '([^']+)' instead$").captures(desc)
            {
        ...
        match member {
            ...
            Member::Properties(params) => {
                if let Element::Type(ty) = self {
                    if ty.item.is_some() {
                        bail!(
                            "Type {} can't have additional properties section",
                            ty.name
                        )
                    } else {
