...
/// apply. `JsAction` is the JS-friendly form the spec layer emits via JSON.
...
pub trait InterfaceDriver: Send {
    ...
    ) -> impl std::future::Future<Output = Result<Vec<Snapshot>>> + Send;

    /// Convert a JS-emitted action (as deserialized from the spec) into a
    /// validated internal action.
